 ...
 * This script thoroughly tests the chat history functionality
 * with AI/LLM/ML topics to ensure history is maintained properly
 */
...
const AUTH_TOKEN =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI5NzdhMzY4ZS1hYmZlLTRhMzgtYWRiZS00NjI1Y2NlOGQ1MDAiLCJlbWFpbCI6ImFkbWluQGV4YW1wbGUuY29tIiwicm9sZSI6IlVTRVIiLCJpYXQiOjE3NjcwNjM1NTYsImV4cCI6MTc2NzA3MjU1Nn0.VE3DU3Gh5e36863pMi0bnadMeffdXttsL8dvcLBByG4';
...
async function test2_FollowUpQuestion(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
  console.log(`TEST 2: Follow-up Question - Testing History Context`);
  console.log(`${'='.repeat(80)}${colors.reset}\n`);

  await wait(2000); // Wait 2s between requests
...
async function test3_DeepFollowUp(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test4_ReferenceEarlier(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test5_NewTopicInSameChat(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test6_MultiTopicReference(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test7_TechnicalDeepDive(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test8_VerySpecificFollowUp(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test9_NewChatSession() {
  ...
  const response = await sendChatMessage('Ban vua noi gi ve overfitting?');
...
async function test10_ComplexLLMConcepts(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test11_TransformerFollowUp(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function test12_LongChainReference(chatId: string) {
  console.log(`\n${colors.bright}${colors.magenta}${'='.repeat(80)}`);
...
async function runAllTests() {
  ...
  console.log(
    `║${' '.repeat(20)}CHAT HISTORY COMPREHENSIVE TEST${' '.repeat(25)}║`,
  );
  console.log(`║${' '.repeat(78)}║`);
  console.log(
    `║${' '.repeat(15)}Testing AI/LLM/ML Knowledge Base${' '.repeat(30)}║`,
  );
  console.log(`╚${'═'.repeat(78)}╝`);
  ...
  try {
    ...
    if (!chatId) {
      console.error(
        `${colors.red}❌ Failed to get chatId from first message${colors.reset}`,
      );
      return;
    }
